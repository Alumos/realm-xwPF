...
get_latest_realm_version() {
    ...
    local latest_version=$(curl -sL --connect-timeout $SHORT_CONNECT_TIMEOUT --max-time $SHORT_MAX_TIMEOUT "$REALM_RELEASE_BASE" 2>/dev/null | \
        head -2100 | \
...
install_realm() {
    ...
    if [ -n "$local_package_path" ] && [ -f "$local_package_path" ]; then
        ...
        DOWNLOAD_URL="$REALM_RELEASE_BASE/download/${LATEST_VERSION}/realm-${ARCH}.tar.gz"
